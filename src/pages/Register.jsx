...
export function Register() {
  ...
  return (
    <Layout>
      ...
      <section className="py-10 w-full flex items-center justify-center min-h-screen bg-cover bg-center" style={{ backgroundImage: "url('assets/nikaah.webp')" }}>
            ...
            <Card color="gray-200" shadow={true} className="flex flex-col md:flex-row overflow-hidden w-full">
              <div className="flex flex-col md:flex-row w-full">
                <div className="md:w-1/2  items-center justify-center hidden md:block  overflow-hidden">
                  <img className="max-w-full max-h-full object-contain" src="assets/nikah.webp" alt="Nikah Muslim Groom and Bride" />
                    ...
                    <div className="mb-1 flex flex-col gap-5">
                      ...
                      <div className="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-4">
                        <div className="w-full md:w-1/2">
                          ...
                          <Input name="password" type="password" maxLength="13" minLength={6} required placeholder="password@123" onChange={handleInputChange} />
                        </div>
                        <div className="w-full md:w-1/2">
                          <label className="block text-sm font-medium text-gray-700">Occapasion <span className="text-red-600">*</span> </label>
                          <Select name="occupation" options={success?.Occupation?.map((item) => ({ value: item?.title, label: item?.title }))} required onChange={(option) => handleSelectChange("occupation", option.value)} />
                        </div>
                      </div>
                      <div className="flex flex-col">
                        <div className="w-full  md:w-4/7">
                          <label className="block text-sm font-medium text-gray-700">Email <span className="text-red-600">*</span> </label>
                          <Input name="email" placeholder="name@mail.com" required onChange={handleInputChange} />
                        </div>
                        <div className="w-full mt-4">
                          <div className="flex items-center  mb-4">
                            <label className="block text-sm font-medium text-gray-700">Gender <span className="text-red-600">*</span> </label>
                            <div className="flex gap-10">
                              <Radio name="gender" label="Male" color="blue" onChange={() => handleSelectChange("gender", "Male")} required />
                              <Radio name="gender" label="Female" color="pink" onChange={() => handleSelectChange("gender", "Female")} required />
                            </div>
                          </div>
                        </div>
                        <div className='flex ites-center space-x-3' >
                          ...
                          <div className='w-2/6'>
                            ...
                            <Select name="height" required options={success?.Height?.map((item) => ({ value: `${item?.height_cm}`, label: ` ${item?.height_feet}ft  ${item?.height_inches}in - ${item?.height_cm}cm ` }))} onChange={(option) => handleSelectChange("height", option.value)} className="w-full capitalize"
                          ...
                          <div className='w-2/6'>
                            ...
                            <Input name="pincode" maxLength="6" minLength={6} required placeholder="123456" className="" onChange={(e) => pin(e)} />
                        ...
                        <div className="w-full text-right">
                          <span className="text-green-200"> {`${formData.city}  `}, {`${formData.state}  `}, {`${formData.country}  `}</span>
                        </div>
                        <div className="w-full text-center mt-4">
                          <Button type="submit" size="lg" color="green"> Submit Registration </Button>
                        </div>
                      </div>
                    </div>
                  </form>
