  ...
  const InterestStatus = async (matchId, status) => {
    try {
      ...
      if (!matchId || status === undefined) {
        toast.error(
          "Match ID and status are required to update interest status"
        );
        return;
      }
      ...
      toast.success(
        response.data.message || "Interest status updated successfully"
      );
      fetchInterests();
    } catch (err) {
      // Extract and display the server error message or fallback to a default message
      const errorMessage =
        err.response?.data?.message || "Error updating interest status";
      ...
      >
        ...
        <CardBody className="flex-1 p-2">
          ...
          <Typography color="gray" className="text-xs font-semibold sm:text-sm">
            ...
            {interest.interested_to_user?.occupation}
          </Typography>
          <Typography color="gray" className="font-semibold text-xs sm:text-sm">
            <FaMosque className="inline text-green-400 mr-1" />{" "}
            {interest.interested_to_user?.caste},{" "}
            {interest.interested_to_user?.sub_caste}
          </Typography>
        </CardBody>
                  ...
                  {interests.new.map((item, index) => (
                    ...
                    >
                      ...
                      <CardBody className="flex-1 p-2">
                        ...
                        >
                          <FaBriefcase className="inline text-green-400 mr-1" />{" "}
                          {item.interested_from_user?.occupation}
                        </Typography>
                        <Typography
                          color="gray"
                          className="font-semibold text-xs sm:text-sm"
                        >
                          <FaMosque className="inline text-green-400 mr-1" />{" "}
                          {item.interested_from_user?.caste},{" "}
                  ...
                  {interests.send.map((item, index) => (
                    ...
                    >
                      ...
                      <CardBody className="flex-1 p-2">
                        ...
                        >
                          <FaBriefcase className="inline text-green-400 mr-1" />{" "}
                          {item.interested_to_user?.occupation}
                        </Typography>
                        <Typography
                          color="gray"
                          className="font-semibold text-xs sm:text-sm"
                        >
                          <FaMosque className="inline text-green-400 mr-1" />{" "}
                          {item.interested_to_user?.caste},{" "}
                          {item.interested_to_user?.sub_caste}
                        </Typography>
                      </CardBody>
                      <div className="flex flex-col items-end gap-2 mt-4 me-4 sm:mt-0">
                        <Button
