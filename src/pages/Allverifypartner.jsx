...
function Allverifypartner() {
    ...
    useEffect(() => {
        const fetchPartners = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${URL}/api/verified-partner?page=${currentPage}`, {
                    headers: {
    ...
    return (
        ...
        <div className='mt-10'>
            ...
            <div className="flex justify-center mt-4">
                <Button onClick={() => setCurrentPage(currentPage - 1)} disabled={currentPage === 1} className="mr-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Previous
                </Button>
                <div className="flex mx-2">
                    {[...Array(totalPages)].map((_, index) => (
                        <Button key={index} onClick={() => setCurrentPage(index + 1)} className={`mx-1 ${currentPage === index + 1 ? 'bg-blue-700' : 'bg-blue-500'} hover:bg-blue-700 text-white font-bold py-2 px-4 rounded`}>
                            {index + 1}
                        </Button>
                    ))}
                </div>
                <Button onClick={() => setCurrentPage(currentPage + 1)} disabled={currentPage === totalPages} className="ml-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Next
                </Button>
            </div>
        </div>
...
export default Allverifypartner
